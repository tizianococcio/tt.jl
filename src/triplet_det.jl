...
"""
simulation without randomness
"""
#this file is part of litwin-kumar_doiron_formation_2014
#Copyright (C) 2014 Ashok Litwin-Kumar
#see README for more information
function sim_det(weights::Matrix{Float64},
    ...
    dua=true # detectors update always
    )
	...
	for cc = 1:Ncells
		v[cc] = vre + (vth0-vre) # Compute menbrane voltage of neuron
		if cc <= Ne 					# Is the neuron an E neuron?
			rx[cc] = rex				# rate of external input
			nextx[cc] = 1/rx[cc]	# time of next external excitatory input becomes smaller if rate of external input is larger
			for dd = 1:Ne
	...
	println("starting simulation v$SIM_VER [with traces, deterministic]")
