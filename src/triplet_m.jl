...
function sim_mall(weights::Matrix{Float64},
	popmembers::Matrix{Int64},
	spikes,#::SpikeTimit.FiringTimes,
	transcriptions::SpikeTimit.Transcriptions,
	net::LKD.NetParams,
...
post = []
for i in 1:Ne
	push!(pre, findall(weights[i,1:Ne] .!= 0.0))
	push!(post, findall(weights[1:Ne,i] .!= 0.0))
end

# each neuron has a matrix where rows = time steps, cols = presynaptic weights
container_pre = Vector{Matrix{Float64}}(undef, Ne)
container_post = Vector{Matrix{Float64}}(undef, Ne)
for i = 1:Ne
    container_pre[i] = zeros(trace_size, length(pre[i]))
    container_post[i] = zeros(trace_size, length(post[i]))
end
...
if learning
	...
	if mod(t, save_traces_timestep) == 0
		for i in 1:Ne
			container_pre[i][q,:] = weights[i, pre[i]]
			container_post[i][q,:] = weights[post[i], i]
		end
		do1[q] = o1[1]
		dr1[q] = r1[1]
		do2[q] = o2[1]
...
tt.save_weight_traces(folder, container_pre, container_post)
...
function sim_mall_full(weights::Matrix{Float64},
	popmembers::Matrix{Int64},
	spikes,#::SpikeTimit.FiringTimes,
	transcriptions::SpikeTimit.Transcriptions,
	net::LKD.NetParams,
...
println("starting simulation v$SIM_VER [with full-matrix traces]")
# trackers for 3 neurons
#voltage_neuron_1_tracker = 0.0*Vector{Float64}(undef,Nsteps)
#adaptation_current_neuron_1_tracker = 0.0*Vector{Float64}(undef,Nsteps)
#adaptive_threshold = 0.0*Vector{Float64}(undef,Nsteps)
...
container_weights = Vector{Matrix{Float64}}(undef, trace_size)
for i=1:trace_size
	container_weights[i] = zeros(Ne, Ne)
end
...
if learning
	...
	if mod(t, save_traces_timestep) == 0
		container_weights[q] = weights[1:Ne, 1:Ne]
		do1[q] = o1[1]
		dr1[q] = r1[1]
		do2[q] = o2[1]
		dr2[q] = r2[1]		
...
tt.save_weight_traces(folder, container_weights)
...
function sim_m_eSTDPoff(weights::Matrix{Float64},
	popmembers::Matrix{Int64},
	spikes,#::SpikeTimit.FiringTimes,
	transcriptions::SpikeTimit.Transcriptions,
	net::LKD.NetParams,
	...
	println("starting simulation v$SIM_VER [with traces, eSTDP off]")
	# trackers for 3 neurons
	#voltage_neuron_1_tracker = 0.0*Vector{Float64}(undef,Nsteps)
	#adaptation_current_neuron_1_tracker = 0.0*Vector{Float64}(undef,Nsteps)
	#adaptive_threshold = 0.0*Vector{Float64}(undef,Nsteps)
